...
export class PythonEnvironmentService implements vscode.Disposable {
  ...
  private _onDidChangeEnvironment =
    new vscode.Emitter<DidChangeEnvironmentEventArgs>();
  ...
  public async initialize(): Promise<boolean> {
    ...
    const envsExt = vscode.extensions.getExtensionById<PythonEnvironmentApi>(
      PYTHON_ENVS_EXTENSION_ID,
    );
  ...
  private async _initFromPythonExtension(): Promise<void> {
    try {
      const pythonExt = vscode.extensions.getExtensionById(PYTHON_EXT_ID);
      if (!pythonExt) {
        console.warn(
    ...
    vscode.window
      ...
      .then(async (selection) => {
        if (selection === "Disable Environments Extension") {
          ...
          const reload = await vscode.Promise.resolve(vscode.window.showMessage("Setting disabled. Reload VS Code for the change to take effect.", 'more'));
  ...
  public async resolveInterpreterPath(
    ...
    if (userConfiguredPath && userConfiguredPath.trim()) {
      const { resolveInterpreterPath } =
        require("@src/features/utils/interpreterPathResolver");
  ...
  private async _checkAndPromptForSetting(): Promise<void> {
    ...
    if (!useEnvsExtension) {
      ...
      const selection = await vscode.Promise.resolve(vscode.window.showMessage("The Python Environments extension requires 'python.useEnvironmentsExtension' to be enabled for full functionality. " +
          "Enable this setting to use Python environment auto-activation in terminals.", 'warning'));
      ...
      if (selection === enableSetting) {
        ...
        const reloadSelection = await vscode.Promise.resolve(vscode.window.showMessage("Setting enabled. Please reload VS Code for the changes to take effect.", 'more'));
  ...
  public async selectEnvironment(): Promise<void> {
    ...
    try {
      ...
      vscode.Promise.resolve(vscode.window.showMessage("Unable to open Python environment picker. " +
          "Please ensure a Python extension is installed and enabled.", 'error'));
    }
  }
