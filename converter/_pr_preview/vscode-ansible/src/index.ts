...
export async function activate(context: ExtensionContext): Promise<void> {
  try {
    ...
    try {
      _mainEntry = require.resolve('@ansible/ansible-language-server')
    } catch {}
    if (!_mainEntry) {
      try { _mainEntry = require.resolve('@ansible/ansible-language-server/package.json') } catch {}
    }
    try {
    ...
    const createClient = () => {
      const client = new LanguageClient(
        ...
        { module: serverPath || _mainEntry || require.resolve('@ansible/ansible-language-server/package.json'), transport: TransportKind.ipc },
        {
          documentSelector: [{ scheme: 'file', language: 'ansible' }, { scheme: 'file', language: 'yaml' }],
          outputChannelName: 'Ansible language support',
          synchronize: { configurationSection: 'ansible' },
    ...
    context.subscriptions.push(
      commands.registerCommand('ansible.restart', async () => {
    ...
    window.showErrorMessage('ansible error: ' + (e.message || String(e)))
  }
}
