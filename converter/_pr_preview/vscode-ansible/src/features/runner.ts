...
export class AnsiblePlaybookRunProvider {
  ...
  private addEEArgs(commandLineArgs: string[]): boolean {
    ...
    try {
      ...
      vscode.Promise.resolve(vscode.window.showMessage(message, 'error'));
      return false;
    }
  ...
  private async invokeViaAnsiblePlaybook(fileObj?: vscode.Uri): Promise<void> {
    ...
    if (typeof playbookFsPath === "undefined") {
      vscode.Promise.resolve(vscode.window.showMessage(`No Ansible playbook file has been specified to be executed with ansible-playbook.`, 'error'));
      return;
    }

    const validationError = validatePlaybookPath(playbookFsPath);
    if (validationError) {
      vscode.Promise.resolve(vscode.window.showMessage(validationError, 'error'));
      return;
    }
  ...
  private async invokeViaAnsibleNavigator(fileObj?: vscode.Uri): Promise<void> {
    ...
    const playbookFsPath = extractTargetFsPath(fileObj);
    if (typeof playbookFsPath === "undefined") {
      vscode.Promise.resolve(vscode.window.showMessage(`No Ansible playbook file has been specified to be executed with ansible-navigator run.`, 'error'));
      return;
    }
