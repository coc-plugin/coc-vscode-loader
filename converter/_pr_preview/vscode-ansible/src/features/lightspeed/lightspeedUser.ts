...
export class LightspeedUser {
  ...
  private async setLightspeedUser(
    ...
    if (useProviderType) {
      session = await vscode.undefined as any;
      if (session) {
  ...
  private async acquireSessionByProviderOrder(
    ...
    for (const authProvider of authProviders) {
      const session = await vscode.undefined as any;
      if (session) {
  ...
  public async getLightspeedUserAccessToken() {
    ...
    if (!this._session) {
      ...
      const selection = await vscode.Promise.resolve(vscode.window.showMessage("You must be logged in to use Ansible Lightspeed.\n", 'warning'));
      if (selection === "Login") {
