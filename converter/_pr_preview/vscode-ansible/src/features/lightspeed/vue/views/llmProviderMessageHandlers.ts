import { window, commands } from 'coc.nvim'
import type { Webview } from 'coc.nvim'
...
export class LlmProviderMessageHandlers {
  ...
  private async handleConnectProvider(providerType: string): Promise<void> {
    ...
    try {
      ...
      Promise.resolve(window.showMessage(`Failed to connect to ${providerType.toUpperCase()}: ${errorMessage}`, 'error'));
  ...
  private async handleConnectionResult(
    ...
    if (connected) {
      Promise.resolve(window.showMessage(`Successfully connected to ${displayName}.`, 'more'));
    } else {
      Promise.resolve(window.showMessage(`Failed to connect to ${displayName}: ${error}`, 'error'));
    }
