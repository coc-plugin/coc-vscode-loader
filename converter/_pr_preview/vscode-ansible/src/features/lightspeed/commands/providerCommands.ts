...
export class ProviderCommands {
  ...
  private async testProviderConnection(): Promise<void> {
    try {
      ...
      if (!activeProvider) {
        vscode.Promise.resolve(vscode.window.showMessage("No provider is currently active. Please configure a provider first.", 'warning'));
        return;
      }
      ...
      vscode.window.withProgress(
        ...
        async () => {
          ...
          if (status.connected) {
            vscode.Promise.resolve(vscode.window.showMessage(`✅ ${activeProvider.toUpperCase()} provider connected successfully!`, 'more'));
          } else {
            vscode.Promise.resolve(vscode.window.showMessage(`❌ ${activeProvider.toUpperCase()} provider connection failed: ${status.error}`, 'error'));
          }
        },
      );
    } catch (error) {
      vscode.Promise.resolve(vscode.window.showMessage(`Connection test failed: ${error instanceof Error ? error.message : "Unknown error"}`, 'error'));
    }
  }
  ...
  private async configureLlmProvider(): Promise<void> {
    try {
      ...
      vscode.Promise.resolve(vscode.window.showMessage(`Configuration failed: ${error instanceof Error ? error.message : "Unknown error"}`, 'error'));
    }
  }
  ...
  private async switchProvider(): Promise<void> {
    ...
    try {
      ...
      vscode.Promise.resolve(vscode.window.showMessage(`Switched to ${selectedProvider.provider.displayName}`, 'more'));
    } catch (error) {
      vscode.Promise.resolve(vscode.window.showMessage(`Failed to switch provider: ${error instanceof Error ? error.message : "Unknown error"}`, 'error'));
    }
  }
}
