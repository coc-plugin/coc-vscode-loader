...
export class LightSpeedAPI {
  ...
  public async completionRequest(
    ...
    try {
      ...
      if (
        ...
        vscode.Promise.resolve(vscode.window.showMessage("Ansible Lightspeed does not have a suggestion for this input. Try changing your prompt, 'more'));
      ...
      if (!(await this._oneClickTrialProvider.showPopup(mappedError))) {
        vscode.Promise.resolve(vscode.window.showMessage(`${mappedError.message ?? UNKNOWN_ERROR} ${formatErrorDetail(mappedError.detail)}`, 'error'));
      }
      return {} as CompletionResponseParams;
    } finally {
  ...
  public async feedbackRequest(
    ...
    try {
      ...
      if (showInfoMessage) {
        vscode.Promise.resolve(vscode.window.showMessage(errorMessage, 'error'));
      } else {
  ...
  public async contentMatchesRequest(
    ...
    if (!(await this.lightspeedAuthenticatedUser.isAuthenticated())) {
      vscode.Promise.resolve(vscode.window.showMessage("User not authenticated to use Ansible Lightspeed.", 'error'));
