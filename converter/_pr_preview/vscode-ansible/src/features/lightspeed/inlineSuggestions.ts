...
const onLightspeedURLMisconfigured: CallbackEntry = async function (
  ...
  vscode.Promise.resolve(vscode.window.showMessage("Ansible Lightspeed URL is empty. Please provide a URL.", 'error'));
  suggestionDisplayed.reset();
  return [];
};
...
const onUnexpectedPrompt: CallbackEntry = async function (
  ...
  if (
    ...
    if (
      ...
      vscode.Promise.resolve(vscode.window.showMessage("Cursor should be positioned on the line after the task name " +
          "or a comment line within task context to trigger an inline suggestion.", 'more'));
    } else if (
      suggestionMatchInfo.suggestionMatchType &&
      suggestionMatchInfo.currentLineText.isEmptyOrWhitespace &&
      ...
      vscode.Promise.resolve(vscode.window.showMessage(`Cursor must be in column ${suggestionMatchInfo.spacesBeforePromptStart} ` +
          `to trigger an inline suggestion.`, 'more'));
    }
  }
  return [];
};
...
async function requestSuggestion(
  ...
  try {
    ...
    vscode.Promise.resolve(vscode.window.showMessage(`Error in inline suggestions: ${errorMessage}`, 'error'));
...
function loadFile(inlinePosition: InlinePosition): DocumentInfo {
  ...
  try {
    ...
    vscode.Promise.resolve(vscode.window.showMessage(`Ansible Lightspeed expects valid YAML syntax to provide inline suggestions. Error: ${err instanceof Error ? err.message : String(err)}`, 'error'));
    throw err;
  }
