...
export async function activate(context: ExtensionContext): Promise<void> {
  try {
    ...
    try {
      serverPath = require.resolve('ruff')
    } catch {}
    if (!serverPath) {
      const _fallback = require('path').join(__dirname, '..', 'server', 'ruff-{{rust-target}}/ruff')
    ...
    const createClient = () => {
      const client = new LanguageClient(
        ...
        { command: serverPath, args: ['server'] },
        {
          documentSelector: [{ scheme: 'file', language: 'python' }],
          outputChannelName: 'A Visual Studio Code extension with support for the Ruff linter and formatter for Python.',
          synchronize: { configurationSection: 'ruff' },
    ...
    window.showErrorMessage('ruff error: ' + (e.message || String(e)))
  }
}
