...
export async function activate(context: ExtensionContext): Promise<void> {
  try {
    ...
    try {
      serverPath = require.resolve('gopls')
    } catch {}
    if (!serverPath) {
      const _fallback = require('path').join(__dirname, '..', 'server', 'gopls')
    ...
    const createClient = () => {
      const client = new LanguageClient(
        ...
        { command: serverPath, args: [] },
        {
          ...
          outputChannelName: 'Rich Go language support for Visual Studio Code',
          synchronize: { configurationSection: 'go' },
    ...
    context.subscriptions.push(
      commands.registerCommand('go.restart', async () => {
    ...
    window.showErrorMessage('go error: ' + (e.message || String(e)))
  }
}
